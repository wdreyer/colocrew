...
router.put("/editApply", function (req, res, next) {
  const datePost = Date.now();
  if (!req.files) {
    console.log("Edit applying", req.body);
    ...
    Applications.updateOne({ _id: req.body._id }, ApplyEdited).then((data) => {
      if (data === null) {
        return res.json({
          message: "Le camp n'a pas été mis à jour",
          result: false,
        });
      } else {
        Applications.findOne({_id: req.body._id}).then((data) => {
            if (data === null) {
                return res.json({
                  message: "Le camp n'a pas été mis à jour",
                  result: false,
                    ...
                    return res.json({
                        message: "Le camp a été mis à jour",
                        result: true,
                        data : data,
                        });
            }
